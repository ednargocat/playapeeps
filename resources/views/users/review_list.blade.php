<div class="row mt-4 border w-100 rounded">
	<div class="col-md-9 pt-0 rounded-3">
		<div class="row p-4 p-md-0 align-items-center mt-4">
			<div class="col-md-12">
				<a href="{{ url('properties/'.$row_host->properties->id.'/'.$row_host->properties->slug) }}">
	...
	<div class="col-md-3 p-0 mt-4 mb-4">
		...
		<a href="{{ url('users/show/'.$row_host->sender_id) }}">
			<p class="text-center font-weight-700 mb-0">@if($row_host->users->display_name=="") {{ $row_host->users->first_name }} @else {{ $row_host->users->display_name }} @endif</p>
		</a>	
	</div>
</div>