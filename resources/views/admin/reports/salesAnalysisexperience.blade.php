    ...
    <section class="content">
      ...
      <div class="row">
        <div class="col-xs-12">
          <div class="box">
            <div class="box-body">
              <form class="form-horizontal" enctype='multipart/form-data' action="{{ url('admin/sales-analysis-experience') }}" method="GET" accept-charset="UTF-8">
                {{ csrf_field() }}
                    ...
                    @for ($i=1; $i <count($monthYears); $i++)
                      <tr>
                        ...
                        @if($monthlyAvgDiff[$i] > 0)
                          <td style="background-color: #ed3615; color: white;">{!! moneyFormat($default_cur_code->org_symbol, $monthlyAvg[$i] ) !!}</td>
                          <td style="background-color: #ed3615; color: white;"><i class="fa fa-arrow-up"></i> +{{ $monthlyAvgDiff[$i] }}%</td>
                        @elseif($monthlyAvgDiff[$i] == 0)  
                          <td style="background-color: #171622; color: white;">{!! moneyFormat($default_cur_code->org_symbol, $monthlyAvg[$i]) !!}</td>
                          <td style="background-color: #171622; color: white;"><i class="fa fa-arrow-right"></i> {{ $monthlyAvgDiff[$i] }}%</td>
                        @else
                        ...
                        @if($reservationRateDiff[$i] > 0)
                          <td style="background-color: #ed3615; color: white;">{{ $reservationRates[$i] }}%</td>
                          <td style="background-color: #ed3615; color: white;"><i class="fa fa-arrow-up"></i> +{{ $reservationRateDiff[$i] }}%</td>
                        @elseif($reservationRateDiff[$i] == 0) 
                          <td style="background-color: #171622; color: white;">{{ $reservationRates[$i] }}%</td>
                          <td style="background-color: #171622; color: white;"><i class="fa fa-arrow-up"></i> {{ $reservationRateDiff[$i] }}%</td> 
                        @else
