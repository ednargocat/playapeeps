<div class="box">
	<div class="box-body no-padding d-block">
		<ul class="nav nav-pills nav-stacked">
			...
			@if(Helpers::has_permission(Auth::guard('admin')->user()->id, 'manage_email_template'))
				<li {{ isset($list_menu) &&  $list_menu == 'menu-11' ? 'class=active' : ''}}><a href="{{ URL::to("admin/email-template/11")}}">Booking Request</a></li>
			@endif
			
			@if(Helpers::has_permission(Auth::guard('admin')->user()->id, 'manage_email_template'))
				<li {{ isset($list_menu) &&  $list_menu == 'menu-12' ? 'class=active' : ''}}><a href="{{ URL::to("admin/email-template/12")}}">Booking Cancelled by Guest</a></li>
			@endif
		</ul>
	</div>
</div>