    ...
    public function ajax()
    {
        return datatables()
            ...
            ->addColumn('action', function ($message) {
                $edit   = $delete = '';
                $edit   = \Helpers::has_permission(Auth::guard('admin')->user()->id, 'edit_messages') ?'<a href="'.url('admin/send-message-email/'.$message->id).'" class="btn btn-sm svbtn"><i class="fa fa-pencil"></i></a>&nbsp;':'';
          ...
          ->addColumn('message', function ($message) {
                return '<a href="'.url('admin/messaging/host/'.$message->bookings->id).'"><button class="btn btn-sm svbtn"><i class="fa fa-eye"></i> View</button>'.wordwrap($message->message,25,"<br>\n").'</a>';
            })
