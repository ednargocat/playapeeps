...
class ReportsController extends Controller
{
    ...
    public function salesReports(Request $request)
    {
        ...
        if($type=="Experience")
        {
            $data['totalNights'] = $this->report->getNightsExp($startDate, $endDate);
            $data['totalIncome'] = number_format($this->report->getIncomesExp($startDate, $endDate), 2, '.', ',');
            $data['totalReservations'] = $this->report->getReservationsExp($startDate, $endDate);
        
            for ($i=0; $i <count($monthYears); $i++) {
                $startDate     = new Carbon('first day of '.$monthYears[$i]);
                $endDate       = new Carbon('last day of '.$monthYears[$i]);
                $monthlyNights[$i] = (int) $this->report->getNightsExp($startDate, $endDate);
            }
        }
        else
        {
    ...
    public function salesAnalysis()
    {
        ...
        if (isset(request()->property)) {
            $data['properties'] = $property = Properties::where('properties.id', request()->property)->where('properties.type', 'property')->select('id', 'name','type')->get();
        } else {
        ...
        if ($data['allproperties'] != '') {
            $data['propertyName'] = Properties::where('id', $data['allproperties'])->where('type', 'property')->first()->name;
        } else {
    ...
    public function searchProperty(Request $request)
    {
        ...
        if ($str==null) {
            $myresult = Properties::where('status', 'Listed')->where('type', 'property')->select('id', 'name')->take(5)->select('properties.id', 'properties.name AS text')->get();
        } else {
            $myresult         = Properties::where('properties.name', 'LIKE', '%'.$str.'%')->where('type', 'property')->select('properties.id', 'properties.name AS text')->get();
        }
    ...
    public function overviewStats()
    {
        ...
        if (isset(request()->property)) {
            $data['properties'] = Properties::where('properties.id', request()->property)->where('type', 'experience')->select('id', 'name')->get();
        } else {
    ...
    public function salesAnalysisExperience()
    {	
        $data['default_cur_code'] = Currency::where('default', 1)->first();
        $data['yearLists']        = $this->report->getSaleYears();
        ...
        if (isset(request()->property)) {
            $data['properties'] = $property = Properties::where('properties.id', request()->property)->where('properties.type', 'experience')->select('id', 'name')->get();
        } else {
            $data['properties'] = $property = null;
        }
        ...
        for ($i=0; $i <count($monthYears); $i++) {
            $startDate       = new Carbon('first day of '.$monthYears[$i]);
            $endDate         = new Carbon('last day of '.$monthYears[$i]);
            $monthlyAvg[$i]  = $this->report->getMonthlyAvgExp($startDate, $endDate, $data['allproperties']);
 
            $monthlyNights        = (int) $this->report->getNightsExp($startDate, $endDate, $data['allproperties']);
            $countDays            =  $startDate->diffInDays($endDate) + 1;
            $reservationRates[$i] =  number_format((($monthlyNights / $countDays) * 100), 2, '.', ',');
        }
        ...
        if ($data['allproperties'] != '') {
            $data['propertyName'] = Properties::where('id', $data['allproperties'])->where('type', 'experience')->first()->name;
        } else {
            $data['propertyName'] = "All Properties";
        }
        ...
        return view('admin.reports.salesAnalysisexperience', $data);
    }
	...
	public function searchExperience(Request $request)
    {
        ...
        if ($str==null) {
            $myresult = Properties::where('status', 'Listed')->where('type', 'experience')->select('id', 'name')->take(5)->select('properties.id', 'properties.name AS text')->get();
        } else {
            $myresult = Properties::where('properties.name', 'LIKE', '%'.$str.'%')->where('type', 'experience')->select('properties.id', 'properties.name AS text')->get();
        }
     ...
     public function overviewStatsExperience()
    {
        ...
        if (isset(request()->reset_btn)) {
            ...
            $data['to']          = null;
            $data['allproperties']   = '';
            $collections = $this->report->getCountryWiseResevationsExp();
        ...
        $collections = $this->report->getCountryWiseResevationsExp($data['from'], $data['to'], $data['allproperties']);
        ...
        return view('admin.reports.overviewexp', $data);
    }
}
